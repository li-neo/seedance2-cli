...
>
> 文档目标：在满足业务时延和质量目标的前提下，控制模型流量增长、重试放大和上下文膨胀，完成可观测、可暂停、可回滚的模型上量。
>
> 文档状态：实践基线。文中的“平台规则”来自参考资料；“建议默认值”需要结合业务 SLO、模型配额和压测结果校准。

## 1. 核心结论

1. 上量必须同时控制 `TPM`、`RPM`、在途并发和业务灰度比例，不能只控制 QPS。
2. 冷启动或恢复流量时应主动爬坡。平台参考规则显示：较前一分钟增加约 300 万 TPM，或 3 分钟内增长约 20%，可能被识别为突发流量；实际判定会随资源水位动态变化。
3. 客户端建议将流量增长斜率控制在每分钟 200 万至 300 万 TPM 以内。对 TTFT 敏感的在线业务，优先采用更保守的曲线，例如至少用 5 分钟上量到 600 万 TPM。
4. 方舟场景默认仅对 `429 + ServerOverloaded` 和 `429 + RequestBurstTooFast` 做退避重试。参数错误、鉴权错误、内容超限和 VLM 素材不合法不能重试。
5. 所有重试必须重新经过统一限流器，并受重试次数、总时长和重试流量预算约束，避免重试风暴。
6. 上下文管理应采用“固定指令 + 近期对话 + 滚动摘要 + 按需检索”的分层结构。先压缩再截断，截断时必须保留系统约束、当前用户意图、未完成工具调用和关键业务状态。
7. 只有成功率、延时、质量、成本和上下文健康度全部通过门禁，才允许进入下一流量阶段。
...
此时瓶颈是 TPM。若只按 RPM 或 QPS 放量，会提前触发 Token 限流。
